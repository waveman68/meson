...
def cmake_is_debug(env: 'Environment') -> bool:
    if env.coredata.optstore.has_option(OptionParts('b_vscrt')):
        is_debug = env.coredata.optstore.get_value_for(OptionParts('buildtype')) == 'debug'
        if env.coredata.optstore.get_value_for(OptionParts('b_vscrt')) in {'mdd', 'mtd'}:
            is_debug = True
        return is_debug
    else:
        # Don't directly assign to is_debug to make mypy happy
        debug_opt = env.coredata.optstore.get_value_for(OptionParts('debug'))
